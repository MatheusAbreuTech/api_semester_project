from flask import (Blueprint, request)
from turma.turma_service import TurmaService

turmas_blueprint = Blueprint('turmas', __name__)
turmas_service = TurmaService()

@turmas_blueprint.route('/turmas', methods=['GET'])
def get_turmas():
    return turmas_service.get_turmas()

@turmas_blueprint.route("/turmas/<int:turma_id>", methods=['GET'])
def get_turma(turma_id):
    return turmas_service.get_turma(turma_id)

@turmas_blueprint.route("/turmas", methods=['POST'])
def create_turma():
    data = request.json
    return turmas_service.create_turma(data)

@turmas_blueprint.route("/turmas/<int:turma_id>", methods=['PUT'])
def update_turma(turma_id):
    data = request.json
    return turmas_service.update_turma(turma_id, data)

@turmas_blueprint.route("/turmas/<int:turma_id>", methods=['DELETE'])
def delete_turma(turma_id):
    return turmas_service.delete_turma(turma_id)

@turmas_blueprint.route('/turmas/<int:turma_id>/professor/<int:professor_id>', methods=['POST'])
def add_professor(turma_id, professor_id):
    return turmas_service.add_professor(turma_id, professor_id)

@turmas_blueprint.route('/turmas/<int:turma_id>/professor', methods=['DELETE'])
def remove_professor(turma_id):
    return turmas_service.remove_professor(turma_id)

@turmas_blueprint.route('/turmas/<int:turma_id>/alunos/<int:aluno_id>', methods=['POST'])
def add_student(turma_id, aluno_id):
    return turmas_service.add_student(turma_id, aluno_id)

@turmas_blueprint.route('/turmas/<int:turma_id>/aluno/<int:aluno_id>', methods=['DELETE'])
def remove_student(turma_id, aluno_id):
    return turmas_service.remove_student(turma_id, aluno_id)


